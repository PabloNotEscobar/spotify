...
export function FavoriteTrack ({fTrack, active, play, playHandler}: FavoriteTrackProps) {
    ...
    return (
        <div className={"w-full h-[54px] hover:bg-[#1F1F1F] rounded-sm px-4 flex flex-row items-center relative group"} onClick={playHandler}>
            ...
            <div className={`h-10 flex justify-center aspect-square z-0 mr-3`}>
                <img
                    src={`http://localhost:3000${fTrack.track?.image}`}
                    alt={String(track?.id)}
                    key={track?.id}
                    draggable={false}
            ...
            <div className={'w-15/100 h-full flex justify-start items-center text-[#9E9E9E] truncate'}>
                {String((new Date(fTrack.addedAt)).toLocaleString('ru-RU', {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric'
                }))}
